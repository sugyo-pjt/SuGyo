...
def center_scale_at_frame(X, alpha=0.2):
    T = X.shape[0]
    Y = X.copy()
    L_SHOULDER_IDX = 42 + 0
    R_SHOULDER_IDX = 42 + 3

    def fallback_center_scale(t):
    ...
    c_prev = None; s_prev = None
    for t in range(T):
        l = Y[t, L_SHOULDER_IDX]; r = Y[t, R_SHOULDER_IDX]
        if np.isfinite(l).all() and np.isfinite(r).all():
            center = 0.5*(l+r); scale = np.linalg.norm(l[:2]-r[:2])
        ...
        # EMA smoothing
        if c_prev is None:
            c_s = center; s_s = scale
        else:
            c_s = alpha*center + (1-alpha)*c_prev
            s_s = alpha*scale  + (1-alpha)*s_prev
        Y[t] = (Y[t] - c_s) / (s_s if s_s>1e-6 else 1.0)
